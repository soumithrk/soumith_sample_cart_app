package com.ekart.springbootjetty.sample.apis.jetty;
...
import com.ekart.springbootjetty.sample.service.utility.LogLevel;
...
public class AggregatedLifeCycleListener implements LifeCycle.Listener {
      ...
      synchronized (lifeCycleCheckLock) {
         ...
         if (componentsStatus.values().stream().allMatch(v -> v)) {
            ...
            if (failedChecks.isEmpty()) {
               ...
               LOGGER.error(LogLevel.FATAL, "Some deep health checks have failed. Not setting the vip_status: {}",
                     failedChecks);
            }
         }
      }
   ...
   public String toString() {
      ...
      return "AggregatedLifeCycleListener [masterHealthCheck=" + masterHealthCheck + ", lifeCycleCheckLock="
            + lifeCycleCheckLock + ", componentsStarted=" + componentsStatus + "]";
   }
}
